...
void help() {
    std::cout << "\n" << _BOLD << "Description:\n" << _END
                 ...
                 "  -c, --max_cycles\tMaximum number of simulated half-periods before giving up.\n"
                 "  -n, --no_vcd    \tDisable waveform dumping (much faster, needed for long runs).\n"
                 "  -L, --no_cpu_log\tDisable the per-instruction CPU activity log.\n"
...
int main(int argc, char** argv) {
    ...
    bool enable_cpu_log = true;
    long long max_cycles = 1000000;
    ...
    static struct option long_options[] = {
        ...
        {"max_cycles", required_argument, 0, 'c'},
        {"no_vcd", no_argument, 0, 'n'},
        {"no_cpu_log", no_argument, 0, 'L'},
        {"help", no_argument, 0, 'h'},
        {0, 0, 0, 0}
    };

    int option_index = 0;
    while ((opt = getopt_long(argc, argv, "i:d:I:D:v:s:c:nLph", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'i':
            ...
            case 's':
                ...
                break;
            case 'c':
                max_cycles = std::stoll(optarg);
                break;
            case 'n':
                enable_vcd = false;
                break;
            case 'L':
                enable_cpu_log = false;
                break;
            case 'h':
                help();
                return 0;
    ...
    FILE *print_save_file_stream = NULL;
    ...
    for (long long time = 0; time < max_cycles; ++time) {
        top->i_xtal_p = !top->i_xtal_p;

        top->eval();
        if (tfp) tfp->dump(static_cast<uint64_t>(time));
        ...
        if (time%2 == 0 && !top->i_xrst) {
            ...
            // print execution log
            if (output_file_stream && CPU->get_instret() != 0 && CPU->get_instret() != last_instret) {
    ...
    // Close file streams
    if (output_file_stream) fclose(output_file_stream);
    if (print_save_file_stream) fclose(print_save_file_stream);
